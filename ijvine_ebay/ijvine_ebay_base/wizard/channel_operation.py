...
class ChannelOperation(models.TransientModel):
	...
	ijvine_from_date=fields.Datetime.to_string(
		fields.Datetime.from_string(
			fields.Datetime.now()
		) + relativedelta(months=-1)
	)
	ijvine_to_date=fields.Datetime.to_string(
		fields.Datetime.from_string(
			fields.Datetime.now()
		) + relativedelta(minutes=-5)
	)
	from_date=fields.Datetime('From Date',default=ijvine_from_date,help='From Date')
	to_date  =fields.Datetime('To Date',default=ijvine_to_date,help='To Date')
	...
	def _get_ecom_store_domain(self):
		...
		if self._context.get('ijvine_channel_id'):
			res+=[('channel_id','=',self._context.get('ijvine_channel_id'))]
		return res
