...
from odoo.addons.ijvine_ebay_base.tools import parse_float,extract_list as EL
...
class ProductFeed(models.Model):
		...
		for variant in variant_objs:
			...
			if not exists:
				...
				if variant.list_price:
					exists.product_name.ijvine_extra_price = parse_float(
						variant.list_price) - parse_float(template_id.list_price)

					price =parse_float(variant.list_price) #- template_id.list_price
					self.ijvine_change_product_price(
						product_id = exists.product_name,
						price = price,
						channel_id = channel_id
					)
				if context.get('ijvine_qty_update',True) and  variant.qty_available and eval(variant.qty_available) > 0:
					res = self.ijvine_change_product_qty(
	...
	def get_variant_extra_values(self,template_id,variant,channel_id):
		...
		if not variant.ijvine_product_id_type:
			vals['ijvine_product_id_type'] = 'ijvine_upc'
		else:
			vals['ijvine_product_id_type'] = variant.ijvine_product_id_type
		if variant.description_sale:
			vals['description_sale'] = variant.description_sale
		...
		if variant.list_price and template_id:
			vals['ijvine_extra_price'] = parse_float(variant.list_price) - parse_float(template_id.list_price)
		...
		if not exists:
			...
			if not product_exists_odoo:
				...
				if variant.qty_available and eval(variant.qty_available) > 0:
					self.ijvine_change_product_qty(
	...
	def _create_product_lines(self,variant_ids,template_id,store_id,location_id,channel_id):
		...
		return message
	@api.model
	def ijvine_change_product_price(self,product_id,price,channel_id):
	...
	@api.model
	def ijvine_change_product_qty(self,product_id,qty_available,location_id):
	...
	def import_product(self,channel_id):
		...
		if not vals.pop('ijvine_product_id_type'):
			vals['ijvine_product_id_type'] = 'ijvine_upc'
		...
		if state == 'done':
			if match:
				...
				if len(variant_lines):
					context['ijvine_qty_update']=False
