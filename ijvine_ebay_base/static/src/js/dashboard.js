odoo.define('ijvine_ebay_base.backend.dashboard',function (require) {
	'use strict'
	...
	var MultichannelDashboard = AbstractAction.extend({
		...
		on_action (e) {
			...
			switch (action) {
				case 'import':
					return this.do_action('ijvine_ebay_base.open_import_wizard_action')
				case 'export':
					return this.do_action('ijvine_ebay_base.open_export_wizard_action')
				case 'open':
		...
		on_action (e) {
			...
			if (action && instance)
				switch (action) {
					...
					case 'evaluate':
						return this.do_action('ijvine_ebay_base.action_feed_sync', {
							additional_context: {
								default_channel_id: instance,
							},
						})
